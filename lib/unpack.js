...
class Unpack extends Parser {
  ...
  [STRIPABSOLUTEPATH] (entry, field) {
    ...
    // strip off the root before checking for ..
    ...
    if (parts.includes('..') || isWindows && /^[a-z]:\.\.$/i.test(parts[0])) {
      ...
      if (field === 'path' || type === 'Link') {
        ...
        // For symlinks, check if the resolved linkpath escapes the
        // extraction directory. Use parts.join('/') (the stripped,
        // normalized representation) rather than the raw path p.
