...
t.test('GHSA-34x7-hfp2-rc4v hardlink .. escape', t => {
  ...
  t.test('async', t => {
    ...
    new Unpack({
      ...
      // Symlink with .. that escapes cwd should be rejected
      t.throws(() => fs.lstatSync(path.resolve(asyncOut, 'valid_sym')),
        'escaping symlink not created')
      t.ok(warnings.some(w => /escapes extraction directory/.test(w[0])),
        'warned about escaping symlink')
  ...
  t.test('sync', t => {
    ...
    // Symlink with .. that escapes cwd should be rejected
    t.throws(() => fs.lstatSync(path.resolve(syncOut, 'valid_sym')),
      'escaping symlink not created')
    t.ok(warnings.some(w => /escapes extraction directory/.test(w[0])),
      'warned about escaping symlink')
...
t.test('GHSA-9ppj-qmqm-q256 windows drive letter escape with multiple dotdot', t => {
  const dir = testdir()
  const out = path.resolve(dir, 'out')
  mkdirp.sync(out)
  ...
  t.test('async', t => {
    ...
    new Unpack({
      ...
      t.throws(
        () => fs.lstatSync(path.resolve(asyncOut, 'a/winrootdotsescapelink')),
  ...
  t.test('sync', t => {
    ...
    t.throws(
      () => fs.lstatSync(path.resolve(syncOut, 'a/winrootdotsescapelink')),
      'escaping symlink is not created',
    )
    t.end()
  })
...
t.test('GHSA-qffp-2rhf-9h96 windows drive letter with dotdot', t => {
  // A symlink with linkpath 'c:..\foo\bar' should have the root stripped
  // and the resulting symlink should point to '..\foo\bar' (not escape)
  const dir = testdir()
  const out = path.resolve(dir, 'out')
  mkdirp.sync(out)
  ...
  t.test('async', t => {
    ...
    new Unpack({
      ...
      // The c: root should have been stripped
      t.ok(
        warnings.some(w => /stripping.*from absolute/.test(w[0])),
        'warned about stripping root',
      )
      t.end()
    }).end(data)
  })
